...
public class AnchorUIManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (OVRInput.GetDown(OVRInput.RawButton.A) && pointList.Count < 3) {
            Vector3 position = OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch);
            pointList.Add(position);
        }

        if (pointList.Count == 3) {
            PlaceAnchor(pointList);
            pointList.Clear();
        }
    }
    ...
    public void PlaceAnchor(List<Vector3> list)
    {

        Vector3 centerPoint = (list[0] + list[2]) / 2.0f;
        // Vector3 hypotenuse = list[0] - list[2];
        // float adjacentSide = 

        Plane plane = new Plane(list[0], list[1], list[2]);
        Quaternion quaternion = Quaternion.Euler(/*plane.normal*/ 45.0f, 0.0f, 0.0f);

        Instantiate(cube, centerPoint, quaternion).GetComponent<SharedAnchor>();
    }
