...
public class SampleController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (OVRInput.GetDown(OVRInput.RawButton.RIndexTrigger) && pointList.Count < 3) {
            Vector3 position = OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch);
            pointList.Add(position);
        }

        if (pointList.Count == 3) {
            PlaceAnchorAtRoot(pointList);
            pointList.Clear();
        }

    }

    public Plane makePlane(List<Vector3> list) {
    ...
    public void PlaceAnchorAtRoot(List<Vector3> list)
    {
        Log("PlaceAnchorAtRoot: root: " + placementRoot.ToOVRPose().ToPosef());

        Vector3 centerPoint = (list[0] + list[2]) / 2.0f;

        Plane plane = new Plane(list[0], list[1], list[2]);
        Quaternion quaternion = Quaternion.Euler(plane.normal);

        colocationAnchor = Instantiate(anchorPrefab, centerPoint, quaternion).GetComponent<SharedAnchor>();
